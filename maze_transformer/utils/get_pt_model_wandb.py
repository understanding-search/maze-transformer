...
from maze_transformer.training.config import BaseGPTConfig, TrainConfig, ConfigHolder, ZanjHookedTransformer
...
def load_model(config_holder: ConfigHolder, model_path: str, fold_ln: bool = True) -> HookedTransformer:
...
def load_wandb_run(
    ...
    model_cfg: BaseGPTConfig = BaseGPTConfig(name=f"model {run_id}", weight_processing={'are_layernorms_folded': True, 'are_weights_processed': True}, **model_properties)

    # Dataset cfg
    grid_n: int = math.sqrt(wandb_cfg['d_vocab'] - 11) #! Jank 
    assert grid_n == int(grid_n), "grid_n must be a perfect square + 11"  # check integer
    ds_cfg: MazeDatasetConfig = MazeDatasetConfig(name=wandb_cfg.get('dataset_name','no_name'), grid_n=int(grid_n), n_mazes=-1)


    cfg: ConfigHolder = ConfigHolder(model_cfg=model_cfg, dataset_cfg=ds_cfg, train_cfg=TrainConfig(name=f"artifact '{artifact_name}', checkpoint '{checkpoint}'"))
    download_path: Path = Path(output_path)/f'{artifact.name.split(":")[0]}'/f'model.iter_{checkpoint}.pt'
...
def load_wandb_pt_model_as_zanj(
    ...
    model: HookedTransformer; cfg: ConfigHolder
    ...
    if save_zanj_model:
        model_zanj_save_path: Path = Path(output_path) / f"wandb.{model_kwargs['run_id']}.zanj"
