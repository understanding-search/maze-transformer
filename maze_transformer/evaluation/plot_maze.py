...
from maze_transformer.generation.latticemaze import LatticeMaze, CoordArray
...
class MazePlot():
    ...
    """
    UNIT_LENGTH = 14
    DEFAULT_PREDICTED_PATH_COLORS = ["tab:blue", "tab:green", "tab:purple", "tab:orange", "tab:olive", "tab:cyan"]
    ...
    def add_predicted_path(self,
        ...
        """
        if label is None:
            label = "predicted path {path_num}".format( path_num = len(self.predicted_paths) + 1)
        if color is None:
    ...
    def _latticemaze_to_img(self) -> NDArray["row col", bool]:
        ...
        for row in range(self.maze.grid_shape[0]):
            for col in range(self.maze.grid_shape[1]):
                # Draw node
                img[row * self.UNIT_LENGTH + 1 : (row + 1) * self.UNIT_LENGTH, col * self.UNIT_LENGTH + 1 : (col + 1) * self.UNIT_LENGTH] = node_color

                # Down connection
                if self.maze.connection_list[0, row, col]:
                    img[(row + 1) * self.UNIT_LENGTH, col * self.UNIT_LENGTH + 1 : (col + 1) * self.UNIT_LENGTH] = connection_color

                # Right connection
                if self.maze.connection_list[1, row, col]:
                    img[row * self.UNIT_LENGTH + 1 : (row + 1) * self.UNIT_LENGTH, (col + 1) * self.UNIT_LENGTH] = connection_color
    ...
    def _plot_path(self, pf: PathFormat) -> None:
        ...
        if pf.quiver_kwargs is not None:
            ...
            self.ax.plot(*zip(*p_transformed),
                         pf.fmt,
                         lw=pf.line_width,
                         color=pf.color,
                         label=pf.label)
        ...
        self.ax.plot([p_transformed[-1][1]], [p_transformed[-1][0]], "x", color=pf.color)
