...
def example_rgb_pixel_grid():
    return np.array([
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.OPEN, PixelColors.OPEN, PixelColors.OPEN, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.OPEN, PixelColors.WALL, PixelColors.OPEN, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL]
    ], dtype=np.uint8)
...
def test_from_pixel_grid_with_positions(example_rgb_pixel_grid):
    marked_positions = {"start": PixelColors.START, "end": PixelColors.END, "path": PixelColors.PATH}

    connection_list, grid_shape, out_positions = LatticeMaze._from_pixel_grid_with_positions(
    ...
    assert isinstance(out_positions["start"], np.ndarray) and isinstance(out_positions["end"], np.ndarray) and isinstance(out_positions["path"], np.ndarray)
...
def test_find_start_end_points_in_rgb_pixel_grid():
    rgb_pixel_grid_with_positions = np.array([
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.START, PixelColors.OPEN, PixelColors.END, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.OPEN, PixelColors.WALL, PixelColors.OPEN, PixelColors.WALL],
        [PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL, PixelColors.WALL]
    ], dtype=np.uint8)
...
def test_pixels_ascii_roundtrip():
    ...
    for maze_gen_func in GENERATORS_MAP.values():
        ...
        assert maze_pixels.shape == expected_shape, f"{maze_pixels.shape} != {expected_shape}"
        assert all(
...
def test_targeted_solved_maze():
    ...
    for maze_gen_func in GENERATORS_MAP.values():
        ...
        expected_shape: tuple = (n*2+1, n*2+1, 3)
        assert tgt_maze_pixels.shape == expected_shape, f"{tgt_maze_pixels.shape} != {expected_shape}"
        assert all(
...
def test_as_adj_list():
    ...
    assert adj_list_to_nested_set(expected) == adj_list_to_nested_set(
        adj_list
    )
