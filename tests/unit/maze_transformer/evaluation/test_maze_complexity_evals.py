from maze_transformer.evaluation.maze_complexity_evals import MAZE_COMPLEXITY_EVALS, MazeComplexityEvals
...
def test_solution_length_maze_eval():
	...
	dataset: MazeDataset = MazeDataset.from_config(cfg).filter_by.path_length(min_length=2)
...
def test_all_maze_evals_run_only():
	...
	for eval_name, eval_fn in MAZE_COMPLEXITY_EVALS.items():
		for maze in dataset.mazes:
			...
			assert isinstance(result, (float, int)), f"Evaluation {eval_name} did not return a number, got {type(result) = } {result = }"
