...
def test_get_intervals_with_custom_intervals():
    ...
    # expected result
    intervals_mod_batch_size = {"print_loss": 1, "checkpoint": 4, "eval_fast": 2, "eval_slow": 8}


    config = TrainConfig(name="test", optimizer=RMSprop, optimizer_kwargs={"lr": 0.001}, batch_size=batch_size, intervals=intervals)

    for dataset_n_samples, use_defaults in itertools.product([100, None], [True, False]):
        calculated_intervals = config.get_intervals(dataset_n_samples, mod_batch_size=False, use_defaults_if_missing=use_defaults)
        assert isinstance(calculated_intervals, dict)
        assert calculated_intervals == intervals

        calculated_intervals_batched = config.get_intervals(dataset_n_samples, mod_batch_size=True, use_defaults_if_missing=use_defaults)
...
def test_get_intervals_with_custom_counts():
    ...
    # expected result
    intervals_expected = {"print_loss": 50, "checkpoint": 20, "eval_fast": 25, "eval_slow": 10}
    intervals_expected_batched = {"print_loss": 10, "checkpoint": 4, "eval_fast": 5, "eval_slow": 2}

    config = TrainConfig(name="test", optimizer=RMSprop, optimizer_kwargs={"lr": 0.001}, batch_size=batch_size, intervals_count=intervals_count)
