...
def disable_plots_in_script(script_lines: list[str]) -> list[str]:
    ...
    if "matplotlib" in result_str_TEMP:
        ...
        assert mpl_last_import_index != -1, f"matplotlib imports not found! see line {mpl_last_import_index}"
        if mpl_first_usage_index != -1:
            assert mpl_first_usage_index > mpl_last_import_index, f"matplotlib plots created before import! see lines {mpl_first_usage_index}, {mpl_last_import_index}"
        else:
    ...
    if "circuitsvis" in result_str_TEMP:
        ...
        assert cirv_last_import_index != -1, f"circuitsvis imports not found! see line {cirv_last_import_index}"
        if cirv_first_usage_index != -1:
            assert cirv_first_usage_index > cirv_last_import_index, f"circuitsvis plots created before import! see lines {cirv_first_usage_index}, {cirv_last_import_index}"
        else:
